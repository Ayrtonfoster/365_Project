...
def mainAlgo(start_location, end_location, pickup_time):
    ...
    while (i < pickups_completed):
        ...
        #IF both car 1 and car 2's time is greater then the next pickup time         
            ...
            #####
            if(i<299):    
                if(pickup_time[i+1] <= car1_time and pickup_time[i+1] <= car2_time):
                    car1_dist1 = shortestPath(car1_location, start_location[i+1]-1)                             #Find the distance from car 1 to the pickup location
                    car2_dist1 = shortestPath(car2_location, start_location[i+1]-1)                             #Find the distance from car 2 to the pickup location

                    # = car's time + distance to node - time of pickup = wait_time
                    route1 = car1_time + car1_dist - pickup_time[i]             #wait time of passenger for car1 to reach first pickup request 
                    route3 = car1_time + car1_dist1 - pickup_time[i+1]          #wait time of passenger for car1 to reach second pickup request    
                   
                    route2 = car2_time + car2_dist - pickup_time[i]             #wait time of passenger for car2 to reach first pickup request      
                    route4 = car2_time + car2_dist1 - pickup_time[i+1]          #wait time of passenger for car2 to reach second pickup request 

                    #if total wait time of new method is less than wait time of old method
                    if(route2 + route3 < route1 + route4):
                        # update car 1 info such that it will go to pickup location i + 1                    
                        tot_wait_time += (car1_dist1 + (car1_time-pickup_time[i+1]))                   #Add time it took for car to reach passenger to tot_wait_time
                        car1_time += car1_dist1                                                      #Add time required for car1 t reach pickup to cars time  
                        car1_dist1 = shortestPath(start_location[i+1]-1, end_location[i+1]-1)            #Calculate time it takes for car to complate drop off  
                        car1_time += car1_dist1                                                      #Add time take to reach destination to car1 time  
                        car1_location = end_location[i+1]-1                                           #Set car1's location to the drop off location 
                                                                                                #Increment counter to nect pickup requst  

                        # update car 2 info such that it will go to pickup location i
                        tot_wait_time += (car2_dist1 + (car2_time-pickup_time[i]))                   #Add time it took for car to reach passenger to tot_wait_time
                        car2_time += car2_dist1                                                      #Add time required for car1 t reach pickup to cars time  
                        car2_dist1 = shortestPath(start_location[i]-1, end_location[i]-1)            #Calculate time it takes for car to complate drop off  
                        car2_time += car2_dist1                                                      #Add time take to reach destination to car1 time  
                        car2_location = end_location[i]-1                                           #Set car1's location to the drop off location 
                        i+=2                                                                        #Increment counter to nect pickup requst  
        ...
        print("car ride ",i," completed", tot_wait_time)
